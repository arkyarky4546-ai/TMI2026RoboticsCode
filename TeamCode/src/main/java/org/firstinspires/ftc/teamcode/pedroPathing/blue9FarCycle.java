...
public class blue9FarCycle extends OpMode {
    ...
    //TODO: get real poses
    private final Pose startPose = new Pose(9.189,-39.844, 1.585); // Start Pose of our robot. (I think these are the right measurements, as 0 degrees corresponds to facing right the starting x is a bit weird as it depends on where on the line we start)
    private final Pose intakePose = new Pose(11.375, -11.117, 1.569);
    private final Pose shootPose = new Pose(12.292, -51.606, 1.659);
    private final Pose endPose = new Pose(12.913, -34.204, 1.566);


    //paths
    private Path score1;
    private PathChain startToShoot, intakeToShoot, shootToIntake, toEndPose;
        ...
        switch (pathState) {
            ...
            case 4: //go to shoot
                if(actionTimer.getElapsedTimeSeconds() > .05) {
                    intakeAndShoot.setPos(0,intakePos);
                    intakeIndex = false;
                    follower.followPath(intakeToShoot,true);
