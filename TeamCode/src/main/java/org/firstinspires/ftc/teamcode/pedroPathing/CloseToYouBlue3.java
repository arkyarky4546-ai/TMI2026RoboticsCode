...
@Autonomous(name = "CloseToYouBlue3", group = "Autonomous")
public class CloseToYouBlue3 extends OpMode {
    ...
    private final Pose startPose = new Pose(8,-53, Math.toRadians(47)); // Start Pose of our robot. (I think these are the right measurements, as 0 degrees corresponds to facing right the starting x is a bit weird as it depends on where on the line we start)
   ...
   /* private final Pose intakePose1 = new Pose(84, -52, Math.toRadians(90));//this is where we should intake the BALLS idk where it is at this time so change late
    private final Pose acIntakePose1 = new Pose(81, -17 , Math.toRadians(90));
    private final Pose intakePose2 = new Pose(55, -60, Math.toRadians(90));
    private final Pose acIntakePose2 = new Pose(55, -15, Math.toRadians(90));*/
    private final Pose endPose1 = new Pose(30, -48, Math.toRadians(0));
    ...
    public void buildPaths() {//this is where we build the path stuff
        score1 = new Path(new BezierLine(startPose, endPose1));
        score1.setLinearHeadingInterpolation(startPose.getHeading(), endPose1.getHeading());
    ...
    public void autonomousPathUpdate() throws InterruptedException {//we can add a lot more paths
        switch (pathState) {
            ...
            case 1:
                ...
                if(shooter2.getVelocity()>(.90*TargetVelocity)){
                    ...
                    else if (indexForSpindex ==2 && timer123.seconds() > 3){
                        //spoon+=1;
                        smart1.startRotate(actual);
                        //smart2.startRotate120(actual);
                        indexForSpindex = 0;
                    }
                    else if (indexForSpindex ==1 && timer123.seconds() > 6){
                        smart1.startRotate(actual);
                        //spoon+=1;
                        //smart2.startRotate120(actual);
                        indexForSpindex += 1;
